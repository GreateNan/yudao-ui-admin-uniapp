...
import { getdirectory } from '@/api/custom/record'

const props = withDefaults(defineProps<{
  modelValue?: number
  label?: string
  showRoot?: boolean // 是否显示顶级文件夹节点
   majorId?: number;
    projectId?: number;
    type?: number;
}>(), {
...
watch(
  () => props.modelValue,
  (val) => {
    // 0 或 undefined 都视为顶级文件夹（如果允许显示顶级）
...
async function loadDeptList() {
  deptList.value = await getdirectory({majorId:props.majorId,projectId:props.projectId,type:props.type})
